...
The function $f: A \rightarrow B$ has domain $A$ and codomain $B$ in $\NN$ for all $P_j$ and $j$ in the type vectors.

\begin{equation*}
    f([1^{P_1}2^{P_2}\cdots n^{P_n}]) = [1^{n-k}2^{P_1}3^{P_2}\cdots (n+1)^{P_n}]
\end{equation*}

Where $k = \sum_{j=1}^{n} P_j$.

\medskip
To prove the function is injective, or one to one, we suppose that $a_1, a_2 \in A$ and $a_1 \neq a_2$. The function $f: A \rightarrow B$ adds $1$ to each value of $j \in a_1 \text{ or } a_2$. Since $a_1 \neq a_2$ then $a_1 + 1 \neq a_2 +1$ as such $f(a_1) \neq f(a_2)$ and the function is injective.

\medskip
To prove the function is surjective, or onto, we suppose that $b \in B$. The inverse of the function $f$ is $g$. Where for each $g(b) = a$ for $f(a) = b$. The function of $g$ is shown below.
