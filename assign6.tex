...
\begin{enumerate}[(a)]
    ...
    \end{align*}
    Calculating the double intersection sequences \textbf{Note: Need to add finishing at B}:
    \begin{align*}
    N_{\ge}(I_a I_d) = \binom{5}{2}\binom{14-5}{9-2} = 360 \quad \quad & N_{\ge}(I_b I_c) = \binom{5}{4}\binom{9-5}{7-4} = 20 \\
    N_{\ge}(I_b I_d) = \binom{5}{4}\binom{14-5}{9-4} = 630 \quad \quad & N_{\ge}(I_c I_d) = \binom{9}{7}\binom{14-9}{9-7} = 360
    \end{align*}
    Calculating the lone triple intersection sequence:
    \begin{equation*}
        N_{\ge}(I_b I_c I_d) = \binom{I_{bt}}{I_{br}}\binom{I_{ct}-I_{bt}}{I_{cr}-I_{br}}\binom{I_{dt}-I_{ct}}{I_{dr}-I_{cr}}\binom{A_t}{A_r} = \binom{5}{4}\binom{9-5}{7-4}\binom{14-9}{9-7}\binom{16-14}{10-9}
    \end{equation*}
    As such $N_{\ge}(I_b I_c I_d) = 5 \cdot 4 \cdot 10 \cdot 2 = 400$. The final answer is:
    \begin{equation*}
        N_{=}(\emptyset) = 8008 - (1650 + 2310 + 1260 + 4004) + (360 + 20 + 630 + 360) - 200 = xxx
    \end{equation*}
    \textbf{Okay okay, Claude said I was wrong because I need to account for getting all the way across. So basically start at A, then go to $I_b$, then go to $I_c$ then go to $I_d$ and finally finish at $B$. So I need to add on the terms for finishing at $B$, which I don't have currently.}
    \item Need to start...
\end{enumerate}
