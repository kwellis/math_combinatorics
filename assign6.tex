...
\textbf{Answer:} 
\begin{enumerate}[(a)]
    \item To get from $A$ to $B$ requires moving across $16$ points. Each one of those points you can make two decisions, either move right $1$ or move up $0$. To reach your desired endpoint requires you to make exactly $10$ right moves or exactly $6$ up moves. So you have this many paths to choose from:
    \begin{equation*}
    ...
    \end{equation*}
    \item Need to describe the four congested intersections. They will be described as the total number of moves, $I_{jt}$ and number of right moves, $I_{jr}$. The number of ups required is $I_{ju} = I_{jt} - I_{jr}$
    \begin{enumerate}[(i)]
        \item $I_a :=$ Total: $5$, rights: $2$, ups: $3$, Math $\binom{5}{2} = 10$
        \item $I_b :=$ Total: $5$, rights: $4$, ups: $1$, Math $\binom{5}{4} = 5$
        \item $I_c :=$ Total: $9$, rights: $7$, ups: $2$, Math $\binom{9}{7} = 36$
        \item $I_d :=$ Total: $14$, rights: $9$, ups: $5$, Math $\binom{14}{9} = 2002$ 
    \end{enumerate}
    The set $P = \{I_a, I_b, I_c, I_d\}$ and the possible subsets is the power set of $P$, $\mathscr{P}(P)$. A secondary intersection, $I_2$, can be accessed from a primary intersection, $I_1$ only if the following is true.
    \begin{equation*}
    ...
    \begin{equation*}
        J = \{\emptyset, \{I_a\}, \{I_b\}, \{I_c\}, \{I_d\}, \{I_aI_d\}, \{I_bI_c\}, \{I_bI_d\}, \{I_cI_d\}, \{I_b I_c I_d\}\}
    \end{equation*}
    As seen, no subset with both $I_a I_b$ or $I_a I_c$ is occuring inside $J$. To mathematically account for landing on a secondary intersection after a primary intersection. You need to subtract the number of moves required to reach the primary intesection from the secondary intersection. If only landing on one intersection, you still need to account for finishing at $B$, so the total possible moves is:
    \begin{align*}
    N_{\ge}(I_a) = \binom{5}{2}\binom{16-5}{10-2} = 1650 \quad \quad & N_{\ge}(I_b) = \binom{5}{4}\binom{16-5}{10-4} = 2310 \\
    N_{\ge}(I_c) = \binom{9}{7}\binom{16-9}{10-7} = 1260 \quad \quad & N_{\ge}(I_d) = \binom{14}{9}\binom{16-14}{10-9} = 4004
    \end{align*}
    Calculating the double intersection sequences:
    \begin{align*}
    N_{\ge}(I_a I_d) = \binom{5}{2}\binom{14-5}{9-2}\binom{16-14}{10-9} = 720 \quad \quad & N_{\ge}(I_b I_c) = \binom{5}{4}\binom{9-5}{7-4}\binom{16-9}{10-7} = 700 \\
    N_{\ge}(I_b I_d) = \binom{5}{4}\binom{14-5}{9-4}\binom{16-14}{10-9} = 1260 \quad \quad & N_{\ge}(I_b I_d) = \binom{9}{7}\binom{14-9}{9-7}\binom{16-14}{10-9} = 720
    \end{align*}
    Calculating the lone triple intersection sequence:
    \begin{equation*}
        N_{\ge}(I_b I_c I_d) = \binom{I_{bt}}{I_{br}}\binom{I_{ct}-I_{bt}}{I_{cr}-I_{br}}\binom{I_{dt}-I_{ct}}{I_{dr}-I_{cr}}\binom{A_t - I_{dt}}{A_r - I_{dr}} = \binom{5}{4}\binom{9-5}{7-4}\binom{14-9}{9-7}\binom{16-14}{10-9}
    \end{equation*}
    As such $N_{\ge}(I_b I_c I_d) = 5 \cdot 4 \cdot 10 \cdot 2 = 400$. The final answer is:
    \begin{equation*}
        N_{=}(\emptyset) = 8008 - (1650 + 2310 + 1260 + 4004) + (720 + 700 + 1260 + 720) - 400 = 1784
    \end{equation*}
    
    \item The clean answer for the problem is:
    \begin{equation*}
        N_{\le}(1) = N_{=}(\emptyset) + N_{=}(1)
    \end{equation*}
    We already have $N_{=}(\emptyset)$ from part b. We now need to compute $N_{=}(1)$ and add it to part b. $N_{=}(1)$ has four separate parts.
    \begin{equation*}
        N_{=}(1) = N_{=}(I_a) + N_{=}(I_b) + N_{=}(I_c) + N_{=}(I_d)
    \end{equation*}
    Each part of $N_{=}(1)$ is calculated with the general inclusion-exclusion formula. Where:
    \begin{equation*}
        N_{=}(S) = \sum_{J: S \subseteq J \subseteq P} (-1)^{|J|-|S|} N_{\ge}(J)
    \end{equation*}
    The addition of $|S|$ in the general inclusion-exclusion formula swaps the signs of $+$ or $-$:
    \begin{align*}
    N_{=}(I_a) &= N_{\ge}(I_a) &&- N_{\ge}(I_aI_d) && && && \\
    N_{=}(I_b) &= N_{\ge}(I_b) && &&- N_{\ge}(I_bI_c) &&- N_{\ge}(I_bI_d) && &&+ N_{\ge}(I_bI_cI_d) \\
    N_{=}(I_c) &= N_{\ge}(I_c) && &&- N_{\ge}(I_bI_c) && &&- N_{\ge}(I_cI_d) &&+ N_{\ge}(I_bI_cI_d) \\
    N_{=}(I_d) &= N_{\ge}(I_d) &&- N_{\ge}(I_aI_d) && &&- N_{\ge}(I_bI_d) &&- N_{\ge}(I_cI_d) &&+ N_{\ge}(I_bI_cI_d)
    \end{align*}
